			...
			<ul class="navbar-nav">
				...
				<li class="nav-item">
					<a href="#">
						...
						<a href="userAD/index.php">Quản lý người dùng</a>
					</a>
				</li>
				<li class="nav-item">
					<a href="#">
				...
				<section class="content-page-right">
					...
					<form class="hot-product-admin">
						<div class="left-hot">
							...
							$sql = "SELECT products.product_name, products.product_img, products.product_price, SUM(order_details.quantity) as total_quantity 
									FROM order_details
							...
							if (mysqli_num_rows($result) > 0) {
								...
								while ($row = mysqli_fetch_assoc($result)) {
									echo "<img class='product-image' src='productAD/upload/" . htmlspecialchars($row['product_img']) . "' alt='Image of " . htmlspecialchars($row['product_name']) . "'>";
								}
								echo "</table>";
							} else {
						...
						<div class="mid-hot">
							...
							// Lấy số lượng đơn hàng của 10 sản phẩm bán chạy nhất
							$sql = "SELECT products.product_name, products.product_img, products.product_price, SUM(order_details.quantity) as total_quantity 									FROM order_details
									INNER JOIN products ON order_details.product_id = products.id
									INNER JOIN orders ON order_details.order_id = orders.id
									INNER JOIN details ON details.order_id = orders.id
									WHERE details.status = 'Hoàn thành'
							...
							if (mysqli_num_rows($result) > 0) {
								...
								while ($row = mysqli_fetch_assoc($result)) {
									if (isset($row["product_price"])) {
										...
										echo "Giá sản phẩm chưa được cập nhật";
									}
								}
								echo "</table>";
							} else {
			...
			<article>
				<section class="information-page">
					<div>
						...
						<input type="range" id="growth-rate" name="growth-rate" min="0" max="100" value="1.60">
